...
- `main` 必须始终保持可公开发布；个人化、实验性或方向不确定的改动按下文“Git 与发布”的分支确认规则处理。
...
- 项目设置中的 `counts_for_coding` 默认开启；关闭后仅排除 coding 热力图与今日／本周／本月／今年提交总数，全部历史按当前开关重算。项目数量仍表示管理的项目总数，项目、同步和历史记录保持原有行为；重新开启恢复统计。
...
- 实验性、个人化、大改动或方向不确定时，若当前任务尚无覆盖该改动的明确分支决定，先确认 `main` 还是分支；这些条件优先于“明确功能可直接进 main”的一般规则。未命中这些条件时，文档、小型修复和明确功能可直接进 `main`。已有分支决定时，同范围的后续提交沿用该决定，不重复询问或擅自切回 `main`；改动范围或公开边界变化时重新判断。

...
- **✅ 每日四项清单** — 追踪 AGENTS.md 更新、Memory 更新、GitHub 推送、部署状态（旧项目兼容 CLAUDE.md）
- **📝 项目笔记 & Obsidian 学习库** — 项目笔记关联每日开发日志；独立知识笔记在 Obsidian 维护和打 tags，工作台提供只读搜索和快捷打开
...
> **按需改造：**「学习」和「自媒体」目前提供的是两套简易示范方案，方便展示如何把个人知识库和内容计划接入工作台。Fork 后可以按自己的记录习惯、工具和工作流自由修改，或者删除不需要的模块。
...
| **Obsidian 归档目录** | 归档每日项目日志，并把 Obsidian 学习库显示在学习页 | 不启用日志归档和学习库索引 |

学习库默认位于 `<vault>/<项目日志总文件夹>/Vibe Coding 学习库/`。其中所有 Markdown 都会显示为笔记，需要分类时使用 Obsidian tags；文件内容始终由 Obsidian 管理，Coding Manager 不会修改它们。

从旧版本升级且数据库里已有全局学习资料时，学习页会显示一次性的“迁移到 Obsidian”入口。迁移不会覆盖同名文件，也不会删除数据库中的旧记录。

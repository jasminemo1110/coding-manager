...
- `db.backup_db()`：用 sqlite backup API 备份 data.db 到 iCloud Drive 的 `coding-dashboard-backups/`（iCloud 不可用退回工程目录 `backups/`），每天一份、同日覆盖、保留最近 30 份；web 每天第一次请求也会触发（`last_backup_date` setting 防重）
